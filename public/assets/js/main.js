...
socket.on('join_room_response', (payload) => {
    ...
    /* if we are being notified of ourselves then ignore message and return*/
...
//doing code for game setup here, part 19
let old_board = [
    [' ',' ',' ',' ',' ',' ',' ',' '],
    [' ',' ',' ',' ',' ',' ',' ',' '],
    [' ',' ',' ',' ',' ',' ',' ',' '],
...
socket.on('game_update', (payload) => {
    ...
    if ( my_color === 'white') {
        ...
        $("#my_color").html('<h3 id="my_color">IDK what i am ERROR</h3>');
    }
    ...
    if ( payload.game.whose_turn === 'white') {
        ...
        $("#my_color").append('<h4>ERROR: dont know whose turn it is</h4>');
    }
    ...
    for (let row = 0; row < 8; row++) {
        for (let column = 0; column < 8; column++) {
            ...
            if(old_board[row][column] !== board[row][column]) {
                ...
                //maybe replace empty to black with something not animated, idk test it
